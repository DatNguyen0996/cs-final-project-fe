...
function ListOfItemsPage() {
  ...
  useEffect(() => {
    dispatch(
      filterProduct({ page: page, data: { ...filterField, special: true } })
    );
  }, [dispatch, productType, filterField, page]);

  const onSubmit = (data) => {
    setFilterField({ ...data, special: true });
    navitate(`/saleOff/filter/${1}`);
    dispatch(filterProduct({ data: { ...data, special: true } }));
  };

  const handleChange = (event, value) => {
    navitate(`/saleOff/filter/${value}`);
  };
