...
function UpdateProduct({ product, page, handleCloseEditBox }) {
  ...
  const onSubmit = async (data) => {
    ...
    await dispatch(updateProduct(createInfor, product._id)).then(() => {
      reset();
    });
  ...
  useEffect(() => {
    setNameProductType(product.productType);
              ...
              <Grid item xs={1} sm={1} md={1}>
                <FSelect
                  ...
                  children={[
                    { value: "Nam", label: "Nam" },
                    { value: "Nữ", label: "Nữ" },
                    { value: "Nam và nữ", label: "Nam và nữ" },
                  ]}
                />
              </Grid>
