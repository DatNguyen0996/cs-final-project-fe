...
import { getAllProduct, filterProduct } from "../features/Product/ProductSlice";
...
function ListOfItemsPage() {
  ...
  let { productTypeParam, pageParam } = useParams();
  ...
  useEffect(() => {
    if (productType === "filter") {
      dispatch(filterProduct({ page: page, data: filterField }));
    } else {
      dispatch(getAllProduct({ page: page, productType: productType }));
      reset();
    }
  }, [dispatch, productType, page, filterField, reset]);
